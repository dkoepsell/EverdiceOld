// Character and inventory API endpoints
...
// API route to get characters for the current user
...
// API route to get a specific character by ID
...
// API route to get inventory for a character
...
// API route to get currency for a character
...
// API route for item rewards
router.post('/characters/:id/rewards/item', async (req, res) => {
  try {
    // Check if user is authenticated and is a DM (can be improved with proper role checks)
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
...
// API route for currency rewards
